...
public class ShiningController : MonoBehaviour
{
    ...
    [SerializeField] MovementDirection rotationDirection = MovementDirection.Left;
    ...
    void HandleMovement()
    {
        transform.DOMove(transform.position + new Vector3(xAxisMoveAmount * (float)rotationDirection, 0, 0), 2).SetEase(Ease.InOutSine).SetLoops(-1, LoopType.Yoyo);
    ...
    void HandlePenalty()
    {
        if (isGoingLeft)
        {
            ...
            {
                leftPenaltyList[i].SetActive(true);
                rightPenaltyList[i].SetActive(false);
            }
        }
        else
        {
            for (int i = 0; i < leftPenaltyList.Count; i++)
            {
                leftPenaltyList[i].SetActive(false);
                rightPenaltyList[i].SetActive(true);
            }
        }
    }
